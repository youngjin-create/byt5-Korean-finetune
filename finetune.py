...
from dataset_nsmc import get_dataset, nsmc_metric
...
class ModelArguments:
    ...
    model_name_or_path: Optional[str] = field(
        default="everdoubling/byt5-Korean-small", metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"}
    )
    task_name: Optional[str] = field(
        default="nsmc", metadata={"help": "Task name", "choices": ["nsmc"]}
    )
...
def main():
    ...
    train_dataset = get_dataset(tokenizer, 'train')
    eval_dataset = get_dataset(tokenizer, 'eval')
    ...
    trainer = Seq2SeqTrainer(
        ...
        compute_metrics=partial(nsmc_metric, tokenizer=tokenizer),
